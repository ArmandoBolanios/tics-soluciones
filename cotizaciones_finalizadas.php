...
FROM cotizacion WHERE auxiliar = 0;
";


if (isset($_POST["propietario"])) {
    $pendiente = $_POST['propietario'];
    if ($pendiente == "Finalizadas") {
        $tabla = '';
        $articulos = 0;
        $venta = 0;
        ...
        while ($cotizacion = $consultaCotizacion->fetch_array(MYSQLI_ASSOC)) {
            ...
            $tabla .= '
                        <td>
                            <a href="detalles_cotizacion_fn.php?claveCotizacion=' . $cotizacion["clave"] . '" class="enlace">ver detalles</a>
                        </td>';
        }
        $tabla .= '
                </tr>
