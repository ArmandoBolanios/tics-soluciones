...
if (isset($_POST["propietario"] )) {
    ...
    while ($ventas = $ejecutarConsultaVentas->fetch_array(MYSQLI_ASSOC)) {
        ...
        $tabla .= '
                <tbody>
                    <tr>
                        ...
                        <td class="desc">' . $ventas["descripcion"] . '</td>
                ';
        ...
        if ($propietarioX == 'Sin Factura') {
            $tabla .= '<td class="sn">' . $ventas["propietario"] . '</td>';
        } else if ($propietarioX == 'Lilia') {
            $tabla .= '<td class="doc">' . $ventas["propietario"] . '</td>';
        } else if ($propietarioX == 'Eduardo') {
            $tabla .= '<td class="inge">' . $ventas["propietario"] . '</td>';
        }
    }
    ...
    <div class="grilla_calendar">
            ...
            <input type="submit" class = "consultaVentas" value="CONSULTAR">
    </div>
    
    ';

    $output = array('tabla' => $tabla,'tfoot' => $tfoot, 'formulario' => $formulario);
